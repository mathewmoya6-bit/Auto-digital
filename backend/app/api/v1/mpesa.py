...
class MpesaService:
    ...
    async def get_payment_status(self, checkout_request_id: str) -> Dict:
        ...
        try:
            logger.info("=" * 60)
            logger.info(f"🔍 Getting payment status for: {checkout_request_id}")

            payment = await self.payment_repo.get_by_checkout_id(checkout_request_id)

            logger.info(f"📊 Payment from repository: {payment}")

            if not payment:
                logger.error("❌ Repository returned None")
                return {
                    "success": False,
                    "error": "Payment not found"
                }

            logger.info(f"✅ Payment found with status: {payment.get('status')}")
