...
NOTE: This version uses httpx.AsyncClient instead of the blocking `requests`
library, so all methods that make HTTP calls are now coroutines and must be
awaited. Callers (e.g. FastAPI routes) need to `await` these methods.
"""
...
# Shared timeouts for the M-Pesa API calls
_AUTH_TIMEOUT = httpx.Timeout(10.0)
_STK_TIMEOUT = httpx.Timeout(30.0)
...
class MpesaService:
    ...
    def get_service_price(self, service_id: str) -> float:
        ...
        NOTE: left synchronous — the supabase client used here is the sync
        client. If you migrate to an async supabase client later, mark this
        `async def` too and await the query.
        """
        try:
    ...
    async def initiate_stk_push(self, phone: str, amount: float,
        ...
        try:
            async with httpx.AsyncClient(timeout=_STK_TIMEOUT) as client:
    ...
    # ─── FIX: Updated get_payment_status with enhanced logging ───
    def get_payment_status(self, checkout_request_id: str) -> Dict[str, Any]:
        """
        Get the status of a payment by checkout_request_id.
        
        Args:
            checkout_request_id: The M-Pesa checkout request ID to look up
            
        Returns:
            Dict containing:
                - success: bool indicating if payment was found
                - status: payment status if found
                - payment data if found
                - error: error message if not found
        """
        try:
            # ─── ADDED: Log the search ───
            logger.info(f"🔍 Searching for payment: {checkout_request_id}")
            ...
            # ─── ADDED: Log what Supabase returned ───
            logger.info(f"📊 Supabase returned: {response.data}")

            if response.data and len(response.data) > 0:
                payment = response.data[0]
                logger.info(f"✅ Payment found: status={payment.get('status')}, amount={payment.get('amount')}")
                ...
                logger.warning(f"⚠️ Payment not found in Supabase: {checkout_request_id}")
                return {
                    'success': False,
            ...
            logger.error(f"❌ Status check error for {checkout_request_id}: {str(e)}")
            return {'success': False, 'error': str(e)}

    # ─── FIX: Updated confirm_payment_manually with enhanced logging ───
    def confirm_payment_manually(self, checkout_request_id: str, user_id: str) -> Dict[str, Any]:
        ...
        Args:
            checkout_request_id: The M-Pesa checkout request ID
            user_id: The user ID to unlock the service for
            
        Returns:
            Dict containing success status and message
        """
        try:
            logger.info(f"🔓 Manual confirm requested: {checkout_request_id} for user {user_id}")
            
            # Check if payment exists first
            logger.info(f"🔍 Searching for payment to confirm: {checkout_request_id}")
            ...
            logger.info(f"📊 Supabase returned for confirm: {response.data}")

            if not response.data or len(response.data) == 0:
                logger.error(f"❌ Payment not found for manual confirm: {checkout_request_id}")
            ...
            logger.info(f"📊 Payment details: service_id={service_id}, user_id={payment_user_id}, status={current_status}")

            # Verify user owns this payment
            if payment_user_id != user_id:
                logger.error(f"❌ User {user_id} does not own payment {checkout_request_id} (owner: {payment_user_id})")
                return {'success': False, 'error': 'Payment does not belong to this user'}

            if not service_id:
                logger.error(f"❌ No service_id found for payment: {checkout_request_id}")
                return {'success': False, 'error': 'Service ID not found'}

            # Check if payment is already completed
            if current_status == 'completed':
                logger.info(f"ℹ️ Payment already completed: {checkout_request_id}")
                # Still try to unlock if not already
                unlocked = self.unlock_service(user_id, service_id, checkout_request_id)
                if unlocked:
                    return {
                        'success': True,
                ...
                return {'success': False, 'error': 'Payment completed but service unlock failed'}
            ...
            if success:
                ...
                logger.error(f"❌ Failed to unlock service for payment: {checkout_request_id}")
                return {
                    'success': False,
            ...
            logger.error(f"❌ Confirm payment error for {checkout_request_id}: {str(e)}")
            return {'success': False, 'error': str(e)}
    ...
    # ─── Get payment history ───
    def get_payment_history(self, user_id: str, limit: int = 50) -> List[Dict[str, Any]]:
