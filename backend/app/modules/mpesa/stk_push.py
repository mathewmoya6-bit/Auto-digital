...
class StkPushService:
    ...
    async def _get_access_token(self) -> str:
        ...
        # ✅ FIX 2: Log the OAuth response before parsing JSON
        logger.info(f"OAuth Status: {response.status_code}")
        logger.info(f"OAuth Headers: {dict(response.headers)}")
        logger.info(f"OAuth Body: {response.text[:500]}...")  # Truncate for logs
        
        # ✅ FIX 3: Don't call response.json() blindly
        ...
        if "access_token" not in data:
            error_msg = data.get("error", data.get("error_description", "Unknown OAuth error"))
