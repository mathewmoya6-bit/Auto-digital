...
# ─── ROUTER ──────────────────────────────────────────────────────────
# ✅ FIXED: Define router BEFORE using it in decorators
...
async def calculate_valuation(
    ...
    try:
        ...
        # Get make and model from the ID if possible
        make = None
        model = None
        try:
            response = (
            ...
            if response.data:
                make = response.data[0].get("make_name")
                model = response.data[0].get("model_name")
        except Exception as e:
            # Silently continue - service will use fallback
            pass
    ...
    try:
        ...
        try:
            response = (
                ...
                .table("vehicle_master_specs")
                .select("make_name, model_name")
                .eq("variant_id", converted_request.vehicle_crsp_id)
                .limit(1)
                .execute()
            )
            if response.data:
        ...
        result = await valuation_service.calculate_valuation(
            ...
            accident_history=converted_request.accident_status,
            location=converted_request.location_name,
