...
from app.modules.running_cost.router import RunningCostRequest, RunningCostResponse
    ...
    def _calculate_five_year_data(self, purchase_price: float, request: RunningCostRequest,
        ...
        for year in range(1, request.years + 1):
            ...
            fuel_cost = (annual_mileage / self._calculate_fuel_efficiency(
                request.engine_size if hasattr(request, 'engine_size') else 1.8,
                request.year, request.trip_type, fuel_type
            )) * request.fuel_price
